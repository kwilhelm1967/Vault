...
async function createZipFile(name, description, files) {
  return new Promise((resolve, reject) => {
    const outputPath = path.join('dist-packages/ready-for-stripe', `${name}.zip`);
...
const getReadmeContent = (plan, price, devices, features) => `# 🔐 Local Password Vault - ${plan}
...
## Support
Email: ${plan.includes('Business') ? 'enterprise@' : plan.includes('Family') || plan.includes('Pro') ? 'priority@' : 'support@'}LocalPasswordVault.com
...
const getExecutableContent = (platform) => `# Local Password Vault ${platform} Application
...
async function createAllZipFiles() {
  try {
    ...
    await createZipFile('LocalPasswordVault-single-user', 'Single User Package', [
      { name: 'README.txt', content: getReadmeContent('Single User', '29.00', '1 device', [
      ...
      { name: 'QUICK_START.txt', content: getQuickStartContent('Single User') },
      { name: 'LocalPasswordVault-Setup.exe', content: getExecutableContent('Windows Installer') },
      { name: 'LocalPasswordVault-Portable.exe', content: getExecutableContent('Windows Portable') },
      { name: 'LocalPasswordVault.dmg', content: getExecutableContent('macOS') },
      { name: 'LocalPasswordVault.AppImage', content: getExecutableContent('Linux') }
    ]);

    // 2. Family Plan ZIP ($49.00)
    await createZipFile('LocalPasswordVault-family-plan', 'Family Plan Package', [
      { name: 'README.txt', content: getReadmeContent('Family Plan', '49.00', '3 devices', [
    ...
    await createZipFile('LocalPasswordVault-pro', 'Pro Package', [
      { name: 'README.txt', content: getReadmeContent('Pro License', '68.00', '6 devices', [
        'Everything in Single User',
        '6 device licenses',
        'Priority support (24h)',
    ...
    await createZipFile('LocalPasswordVault-business-plan', 'Business Plan Package', [
      { name: 'README.txt', content: getReadmeContent('Business Plan', '99.00', '10 devices', [
