...
app.use(helmet());
app.use(cors({
  origin: process.env.ALLOWED_ORIGINS?.split(',') || ['https://localpasswordvault.com'],
  credentials: true
}));
...
app.post('/webhook/stripe', express.raw({type: 'application/json'}), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    console.log(`Checkout completed for ${customerEmail}, product: ${productId}`);
    ...
    switch (productId) {
      ...
      case 'prod_business_plan':
        ...
        break;
      default:
        console.log(`Unknown product ID: ${productId}, defaulting to single user`);
    }
