# Terms of Service

**Last Updated: December 2024**

Please read these Terms of Service ("Terms") carefully before using Local Password Vault ("the Application", "the Software", "the Service").

By downloading, installing, or using Local Password Vault, you agree to be bound by these Terms. If you do not agree, do not use the Application.

---

## 1. Acceptance of Terms

By accessing or using Local Password Vault, you agree to:
- These Terms of Service
- Our Privacy Policy
- Our End User License Agreement (EULA)

If you are using the Application on behalf of an organization, you represent that you have authority to bind that organization to these Terms.

---

## 2. License Grant

### 2.1 Personal License
A Personal License grants you the right to:
- Install and use the Application on one (1) device
- Store unlimited passwords and data
- Receive updates for the license period

### 2.2 Family License
A Family License grants you the right to:
- Install and use the Application on up to five (5) devices
- Share the license with household members
- Receive updates for the license period

### 2.3 Trial License
A Trial License grants you:
- Full access to the Application for 14 days
- One trial per user/device
- No payment required during trial

### 2.4 License Transfers
- You may transfer your license to a new device up to **three (3) times per year**
- Each transfer deactivates the license on the previous device
- Transfers require internet connection for validation
- If you exceed the transfer limit, contact support for assistance
- Transfer count resets annually (12 months from first transfer)

### 2.5 Restrictions
You may NOT:
- Share license keys outside your authorized devices
- Reverse engineer, decompile, or disassemble the Application
- Remove or alter any proprietary notices
- Use the Application for illegal purposes
- Redistribute or resell the Application

---

## 3. Free Trial

### 3.1 Trial Period
- 14-day free trial with full features
- No credit card required
- Automatic expiration (no auto-billing)

### 3.2 Trial Limitations
- One trial per user/device
- Trial abuse may result in service restriction
- Data created during trial is retained if you purchase

---

## 4. Purchases and Payments

### 4.1 Pricing
- Personal License: $49 (one-time)
- Family License: $79 (one-time)
- Prices may change with notice

### 4.2 Payment Processing
- Payments processed by Stripe
- We do not store payment card details
- All prices in USD unless stated otherwise

### 4.3 Refund Policy
- 30-day money-back guarantee
- Request refunds at support@localpasswordvault.com
- Refunds processed within 5-10 business days

---

## 5. Updates and Support

### 5.1 Software Updates
- Updates provided at our discretion
- Updates may add, modify, or remove features
- Critical security updates strongly recommended

### 5.2 Technical Support
- Email support for licensed users
- Response within 2 business days
- Support for current and previous major versions

---

## 6. Data and Privacy

### 6.1 Your Data
- All data stored locally on your device
- We cannot access, view, or recover your data
- You are responsible for your master password
- You are responsible for data backups

### 6.2 Data Loss
We are NOT responsible for data loss due to:
- Forgotten master password
- Device failure or theft
- Uninstallation without backup
- User error

### 6.3 Privacy
See our Privacy Policy for details on data handling.

---

## 7. Security

### 7.1 Encryption
- AES-256-GCM encryption standard
- Zero-knowledge architecture
- Your master password is never transmitted

### 7.2 Your Responsibilities
You agree to:
- Choose a strong master password
- Keep your master password confidential
- Maintain secure backups
- Report security vulnerabilities responsibly

### 7.3 Security Disclaimer
While we implement strong security measures, no system is 100% secure. You use the Application at your own risk.

---

## 8. Intellectual Property

### 8.1 Ownership
Local Password Vault and all associated intellectual property is owned by us. This includes:
- Software code
- User interface design
- Documentation
- Trademarks and logos

### 8.2 Your Content
You retain ownership of all data you store in the Application. We claim no rights to your content.

---

## 9. Disclaimer of Warranties

THE APPLICATION IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND.

WE DISCLAIM ALL WARRANTIES, EXPRESS OR IMPLIED, INCLUDING:
- MERCHANTABILITY
- FITNESS FOR A PARTICULAR PURPOSE
- NON-INFRINGEMENT
- ACCURACY OR RELIABILITY

WE DO NOT WARRANT THAT:
- The Application will be error-free
- The Application will meet your requirements
- Results will be accurate or reliable

---

## 10. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW:

WE SHALL NOT BE LIABLE FOR ANY:
- Indirect, incidental, or consequential damages
- Loss of profits, data, or business opportunities
- Personal injury or property damage
- Damages arising from your use or inability to use the Application

OUR TOTAL LIABILITY SHALL NOT EXCEED THE AMOUNT YOU PAID FOR THE LICENSE.

---

## 11. Indemnification

You agree to indemnify and hold us harmless from any claims, damages, or expenses arising from:
- Your use of the Application
- Your violation of these Terms
- Your violation of any rights of another

---

## 12. Termination

### 12.1 By You
You may terminate by:
- Uninstalling the Application
- Discontinuing use

### 12.2 By Us
We may terminate your license if you:
- Violate these Terms
- Abuse the trial system
- Engage in fraudulent activity

### 12.3 Effect of Termination
Upon termination:
- Your license to use the Application ends
- You must uninstall the Application
- Your locally stored data remains accessible

---

## 13. Governing Law

These Terms are governed by the laws of [Your Jurisdiction], without regard to conflict of law principles.

Any disputes shall be resolved in the courts of [Your Jurisdiction].

---

## 14. Changes to Terms

We may modify these Terms at any time. Changes will be:
- Posted on our website
- Noted in application updates
- Effective upon posting

Continued use after changes constitutes acceptance.

---

## 15. General Provisions

### 15.1 Entire Agreement
These Terms constitute the entire agreement between you and us regarding the Application.

### 15.2 Severability
If any provision is found unenforceable, the remaining provisions remain in effect.

### 15.3 Waiver
Failure to enforce any right does not constitute a waiver.

### 15.4 Assignment
You may not assign these Terms. We may assign freely.

---

## 16. Contact Information

For questions about these Terms:

**Email:** legal@localpasswordvault.com

**Website:** https://localpasswordvault.com

**Support:** support@localpasswordvault.com

---

## Summary of Key Points

| Topic | Summary |
|-------|---------|
| **License** | Personal (1 device) or Family (5 devices) |
| **Transfers** | 3 transfers per year allowed |
| **Trial** | 14 days, full features, no card required |
| **Refunds** | 30-day money-back guarantee |
| **Your Data** | Local only, your responsibility |
| **Our Liability** | Limited to license purchase price |
| **Updates** | Provided, but not guaranteed |

---

*By using Local Password Vault, you acknowledge that you have read, understood, and agree to these Terms of Service.*

